...
function attachResourceDescriptionValidation(input) {
  const update = () => {
    ...
    // Create only enabled when BOTH fields are valid
...
resourceForm.addEventListener("submit", (e) => {
  if (!validateForm()) {
    ...
    console.error("Form invalid — not submitted");
    return;
  }

  // If valid, clean the values before sending
  resourceNameInput.value = resourceNameInput.value.trim();
resourceDescriptionInput.value = resourceDescriptionInput.value.trim();
